...
// Aprroach 2 : https://www.geeksforgeeks.org/cses-solutions-josephus-problem-i/
void solver() {
    ...
    while(child.size() > 1){
        cout << deleteChild << " ";
        child.erase(deleteChild);

        auto nextChild = child.upper_bound(currChild);
        if(nextChild != child.end()){
            currChild = *nextChild;
        }
        else{
            currChild = *child.begin();
        }

        auto deleteNextChild = child.upper_bound(currChild);
        if(deleteNextChild != child.end()){
            deleteChild = *deleteNextChild;
        }
        else{
            deleteChild = *child.begin();
        }
    }
    cout << *child.begin();
}
