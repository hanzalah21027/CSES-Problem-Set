...
void solver() {
    ...
    auto twoSum = [&](int start, int end, ll sum) -> pii{
        while(start < end){
            if(a[start].ff + a[end].ff == sum){
                return {start, end};
            }
            else if(a[start].ff + a[end].ff > sum){
                end--;
            }
            else{
                start++;
    ...
    auto threeSum = [&](int p, int x) -> vector<int>{
        vector<int> res = {-1, -1, -1}; 
        for(int i = p; i < n - 2; i++){
            pii idx = twoSum(i + 1, n - 1, x - a[i].ff);
            if(idx != pair(-1, -1)){
                res = {a[i].ss, a[idx.ff].ss, a[idx.ss].ss};
                return res;
            }
        }
        return res;
    ...
    for(int i = 0; i < n - 3; i++){
            vector<int> res = threeSum(i + 1, x - a[i].ff);
            vector<int> match = {-1, -1, -1};
            if(res != match){
